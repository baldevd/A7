...
//NEEDS TO BE EDITED
	public boolean checkWin() {
		...
		if(_game_won){return true;}

		int countBlacks=0;int countWhites=0;
...
//}
		...
		for(Spot s:_board){if(s.getSpotColor()==Color.WHITE){
		...
		System.out.println("Blacks "+countBlacks);System.out.println("Whites "+countWhites);_noWhite=countWhites;_noBlack=countBlacks;

		int playCol;if(_next_to_play==Player.BLACK){playCol=0;}else{playCol=1;} System.out.println("Legal? "+hasLegalMoves());if((hasLegalMoves()==false)){if(countBlacks>countWhites){_winnerName="Black";}

		if(countWhites>countBlacks){_winnerName="White";}

		if(countWhites==countBlacks){_winnerName=null;}

		System.out.println("GAME WON");_game_won=true;
